...
const Page = () => {
  ...
  toast.update(updToastId,{type:toast.TYPE?.PENDING,autoClose:1000,isLoading: true})
      ...
      fetch('/api/admin/faqs/general', {method: 'PUT',
        headers: { 'Content-Type': 'application/json' },body: JSON.stringify(formData),
      }).then((res) => res.json())
       .then((resp) => {
         if(resp.success){
  ...
  toast.update(delToastId,{type:toast.TYPE?.PENDING,autoClose:1000,isLoading: true})
    ...
    fetch('/api/admin/faqs/general', {method: 'DELETE',
 ...
 useEffect(() => {
  ...
  return (
    ...
    <Popup state={createPopup} setState={setCreatePopup}  >
     <form onSubmit={CreateFaq} className='flex flex-col mx-10 py-5 w-full' >
    ...
    <Popup state={updatePopup} setState={setUpdatePopup}  >
     <form ref={updateFormRef} onSubmit={UpdateFaq} className='flex flex-col mx-10 w-full' >
    ...
    <div className='flex flex-col mx-10' style={{height:'calc(100vh - 100px)'}} > 
     <ActionBtns buttons={[{type:'trigger',text:'Add Faq',trigger:setCreatePopup},{type:'link',text:'Categories',link:'/neu-admin/faqs/general/categories'}]} />
