    ...
    <>
      {data.length > 0 && (
        <div className={`flex flex-col items-center bg-b3/10 py-10 lg:py-14 xl:py-28 ${Style}`}>
          ...
          <div className="maincontainer mt-10 grid grid-cols-2 gap-4 md:gap-6 lg:grid-cols-3 xl:gap-10 2xl:gap-x-10 2xl:gap-y-14">{data && data.length > 0 && data.map((item, index) => <SProductCard key={index} title={item.title} image={item.thumbnail} link={`/products?${urlParam}=${item.slug}`} />)}</div>
