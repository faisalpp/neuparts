  ...
  return (
    <div className="grid grid-cols-1 gap-5 rounded-xl border border-b14 px-5 py-10 sm:grid-cols-[150px_1fr] md:grid-cols-[240px_1fr] xl:grid-cols-[200px_1fr] xl:gap-0 xl:px-2">
      ...
      <div className="space-y-4">
        ...
        <button
          onClick={() => {
            data._id === ID ? dispatch(resetDryer()) : dispatch(setDryer({ id: data._id, title: data.title, isSale: data.isSale, salePrice: data.salePrice, regPrice: data.regPrice, rating: data.rating, bulletDescription: data.bulletDescription, brand: data.brand, tags: data.tags, media: data.media.find((item) => item.file === 'image')?.data }));
          }}
          type="button"
