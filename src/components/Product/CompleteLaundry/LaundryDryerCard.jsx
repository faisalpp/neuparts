...
import { setDryer,resetDryer } from '../../../store/laundarySlice'

const LaundryCard = ({data}) => {

    const dispatch = useDispatch()

    const ID = useSelector((state)=>state.laundary.dryer?.id);
    ...
    return (
        <div className='border grid grid-cols-1 sm:grid-cols-[150px_1fr] md:grid-cols-[240px_1fr] xl:grid-cols-[200px_1fr] xl:gap-0 gap-5 border-b14 rounded-xl py-10 xl:px-2 px-5'>
            ...
            <div className='space-y-4'>
                ...
                <button onClick={()=>{data._id === ID ? dispatch(resetDryer()) :dispatch(setDryer({id:data._id,title:data.title,isSale:data.isSale,salePrice:data.salePrice,regPrice:data.regPrice,rating:data.rating,bulletDescription:data.bulletDescription,brand:data.brand,tags:data.tags,media:data.media.find((item)=>item.file === 'image')?.data}))}} type="button" className={` ${ID === data._id ? 'text-black border-2 border-b7 bg-white':'bg-b7 text-white'} py-3 px-4 flex gap-1 items-center justify-center text-center w-full  rounded-lg text-xs font-bold`}>
                    Select Item
                    <FaArrowRight />
                </button>
            </div>
