  ...
  return (
    <tr className="border-b border-l border-r border-b6 text-xs font-semibold">
      ...
      <td className="whitespace-nowrap px-5 py-3">
        <Link href={`/product/${item.title.toLowerCase().replace(/\s/g, '-')}`} className="text-b6 underline">
          {item.title}
        </Link>
      </td>
      ...
      <td className="whitespace-nowrap px-1 py-3">
        <div className="ml-5 flex w-14 items-center justify-between rounded-md border-[1px] border-b6">
          <BsFillArrowLeftSquareFill onClick={(e) => DecQty(e, item.pid, item.count, productPrice)} className="cursor-pointer text-lg text-b6" /> <span>{item.count}</span> <BsFillArrowRightSquareFill onClick={(e) => IncQty(e, item.pid, productPrice)} className="cursor-pointer text-lg text-b6" />
        </div>
      </td>
