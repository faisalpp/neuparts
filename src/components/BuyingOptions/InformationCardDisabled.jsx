...
const InformationCard = ({rating}) => {
  return (
     <div className='relative flex items-center justify-between bg-[#F8F8F8] p-5 rounded-2xl cursor-not-allowed'>
      <div className="absolute bg-b31/50 flex w-full h-full left-0 rounded-2xl" ></div>
      <h3 className='text-sm capitalize'><span className='font-semibold'>{rating} Star</span> rating</h3>
      <div className='text-sm capitalize'><span className='font-semibold'>0</span> buying options</div>
      <div className='w-120px bg-black/[0.08] rounded-lg' ><span className='flex rounded-lg bg-gradient-to-r from-b32 to-b31 w-1/5 h-2' ></span></div>
      
      <div className='text-b16 font-semibold text-xs px-3 py-[5px] bg-b31 flex items-center rounded-full'>
      ...
      <div className='text-b32 text-xl font-body'>$230.00</div>
     </div>
  )
}
