        ...
        <div className={`border-gray-300} flex w-full cursor-pointer flex-col rounded-xl border-2 bg-white px-1 py-4 hover:border-2 hover:border-b3 ${product.stock ? '' : 'pointer-events-none select-none grayscale'} ${slug === product.slug ? 'border-b3' : 'border-gray-300'}`}>
          ...
          <Link href={`/product/${product?.slug}`}>
            ...
            <div className="relative my-3 flex w-full items-center justify-center">
              <Image onErrorCapture={()=>setThumbnail('/no-image.webp')} width={400} height={400} quality={100} src={thumbnail} className="h-[135px] w-28 object-contain" alt="product" />
            </div>
            <div className="flex flex-col space-y-3 px-2">
              <div className="flex items-center">
                <h6 className="text-sm font-semibold text-b3">${product.regular_price}</h6>
                {product.is_sale ? (
                ...
                <>
                  <div className="flex items-center">
                    ...
                    <div className="flex w-full -translate-y-3 justify-end">
                      <span className="rounded-2xl bg-b4 px-2 py-0.5 text-[9px] font-semibold md:py-1 lg:px-3 lg:text-[8px]">{(((product.regular_price - product.sale_price) / product.regular_price) * 100).toFixed(2)}%</span>
                    </div>
                  </div>
                  <div className="w-full rounded-lg bg-gray-100 maxmd:!mt-1">
                    <span className="flex h-2 rounded-lg bg-gradient-to-r from-b4 to-b7" style={{ width: `${(((product.regular_price - product.sale_price) / product.regular_price) * 100).toFixed(2)}%` }}></span>
                  </div>
                </>
              ) : null}
            </div>
