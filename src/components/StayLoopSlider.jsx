    ...
    <>
      <div className='relative my-8' >
        ...
        <div id="id3" className='flex overflow-x-hidden space-x-3 scroll-smooth' >
          ...
          {loopVideo.length > 0 ? loopVideo.map((item, index) => <div key={index} className='relative' >
            ...
            {/* </div> */}
            {item.type === 'iframe' ? <div className='relative xl:h-[150px] xl:w-[200px] lg:w-[200px] lg:h-32 w-44 h-32 ' ><div onClick={e => HandleParentChange(e, { url: item.url, thumb: item.thumbnail })} className='absolute bg-black/70 cursor-pointer rounded-xl w-full h-full flex z-[99px] justify-center  items-center' >{video === item.url ? <BsFillStopCircleFill className="text-gray-300 text-4xl" /> : <AiFillPlayCircle className="text-gray-300 text-4xl" />}</div><Image width={400} height={400} quality={100} alt={item.thumbnail} src={item.thumbnail} className='xl:h-[150px] xl:w-[200px] lg:w-[200px] lg:h-32 w-44 h-32  rounded-2xl' /></div> : null}
