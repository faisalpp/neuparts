    ...
    <>
      <div className="relative my-8">
        <button onClick={btnprev} className="absolute -right-10 top-0 z-40 h-full">
          <div className="group hidden cursor-pointer rounded-full bg-black/30 px-1 py-1 text-white hover:bg-cyan-500 lg:flex">
          ...
          {loopVideo.length > 0
            ? loopVideo.map((item, index) => (
                <div key={index} className="relative">
                  ...
                  {/* </div> */}
                  {item.type === 'iframe' ? (
                    <div className="relative h-32 w-44 lg:h-32 lg:w-[200px] xl:h-[150px] xl:w-[200px] ">
                      <div onClick={(e) => HandleParentChange(e, { url: item.url, thumb: item.thumbnail })} className="absolute z-[99px] flex h-full w-full cursor-pointer items-center justify-center rounded-xl  bg-black/70">
                        {video === item.url ? <BsFillStopCircleFill className="text-4xl text-gray-300" /> : <AiFillPlayCircle className="text-4xl text-gray-300" />}
                      </div>
                      <Image width={400} height={400} quality={100} alt={item.thumbnail} src={item.thumbnail} className="h-32 w-44 rounded-2xl lg:h-32 lg:w-[200px] xl:h-[150px]  xl:w-[200px]" />
                    </div>
                  ) : null}
                  {item.type !== 'iframe' ? (
                    <div className="relative">
                      ...
                      <video className="h-32 w-32 rounded-2xl lg:h-32 lg:w-[200px] xl:h-[150px] xl:w-[200px] " src={item.url} />
                    </div>
                  ) : null}
