    ...
    <>
      <div className={`group relative flex flex-col rounded-2xl border-2 border-gray-100 bg-white ${sliderstyle ? sliderstyle : 'sm:mx-2'} overflow-hidden`}>
        ...
        <div className="relative flex w-full justify-center px-3 pt-10 lg:px-5 xl:px-5">
          <Image src={product?.image.length > 0 ? product?.image[0].url : '/popular-parts.webp'} width={400} height={400} quality={100} className=" xl:w-54 h-60 w-[160px] object-contain lg:w-52" alt="refrigrator" />
        ...
        <div className="flex flex-col gap-y-3 p-3 lg:p-5 xl:p-6">
          ...
          <div className="flex">
            <Link href={`/product/${product?.slug}`}>
              <h4 className="font-semibold text-b3">${product?.isSale ? product?.sale_price : product?.regular_price}</h4>
            </Link>
            {product?.isSale ? (
